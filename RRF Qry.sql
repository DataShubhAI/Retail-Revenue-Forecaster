...
-- Q.8 Write a SQL query to find the top 5 customers based on the highest total sales 
	...
	select distinct customer_id, sum(total_sale) as highest_sale from retail_sales
	group by 1
	order by 2 desc
	limit 5;
...
-- Q.9 Write a SQL query to find the number of unique customers who purchased items from each category.
	...
	select distinct category, count(distinct customer_id) as unique_cust
	from retail_sales
	group by 1;
...
-- Q.10 Write a SQL query to create each shift and number of orders (Example Morning <=12, Afternoon Between 12 & 17, Evening >17)
	...
	(select sale_time,transactions_id,
			...
			when extract(hour from sale_time) between 12 and 17 then 'After_noon'
			else 'evening'
			end as Shift
	from retail_sales)
